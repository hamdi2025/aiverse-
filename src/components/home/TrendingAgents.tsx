    ...
    <div className="w-full mb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {TRENDING_AGENTS.map((agent, i) => {
          ...
          return (
            <div key={agent.id}
              className="group relative bg-white/5 hover:bg-violet-600/10 border border-white/10 hover:border-violet-500/30 rounded-2xl p-4 flex flex-col gap-3 transition-all duration-200">
              ...
              {/* Trending badge */}
              <span className="text-[9px] font-bold uppercase tracking-wider bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full self-start">
                🔥 {t.trending}
              </span>
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-500 text-[11px] mt-1 line-clamp-2">{agent.description[locale]}</p>
              </div>

              {/* Rating */}
              <div className="flex items-center gap-1">
                <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                <span className="text-white text-xs font-bold">{agent.rating.toFixed(1)}</span>
                <span className="text-gray-600 text-[10px] ml-auto">{agent.views >= 1000 ? `${(agent.views/1000).toFixed(0)}k` : agent.views} views</span>
              </div>
              ...
              <div className="flex gap-1.5">
                <a href={url} target="_blank" rel="noopener noreferrer"
                  ...
                  className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-violet-600/80 hover:bg-violet-600 text-white text-[11px] font-bold rounded-lg transition">
                  Try <ArrowUpRight className="w-3 h-3" />
                </a>
                <Link href={`/${locale}/compare`}
                  className="flex items-center justify-center px-2 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-400 hover:text-white text-[11px] rounded-lg transition">
                  ⚡
                </Link>
              </div>
            </div>
