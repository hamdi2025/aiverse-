    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">Welcome back, {user?.firstName || 'Hamdi'} 👋</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            {MOCK_SUBMISSIONS.map(sub => (
              <div key={sub.id} className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="text-gray-900 font-semibold text-sm">{sub.name}</p>
                ...
                <p className="text-gray-600 text-xs mb-3 line-clamp-1">{sub.desc}</p>
        ...
        {/* Top Tools by Views */}
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
          <h2 className="text-gray-900 font-bold text-lg flex items-center gap-2 mb-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <a key={i} href={action.href} target={action.href !== '#' ? '_blank' : undefined}
              ...
              className="flex flex-col items-center gap-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl p-4 text-center transition">
              <span className="text-2xl">{action.icon}</span>
              <span className="text-gray-900 text-xs font-semibold">{action.label}</span>
            </a>
          ))}
        </div>
      </div>
