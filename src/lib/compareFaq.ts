...
export function getExpertVerdict(t1: AITool, t2: AITool): Record<Locale, string> {
  ...
  return {
    ...
    fr: `Notre avis d'expert : ${tie
      ...
      : `${winner.name} est le choix le plus solide globalement, avec ${winner.rating}/5 contre ${other.rating}/5 pour ${other.name}`}${wStrength ? `, et il se distingue par « ${tl(wStrength, 'fr')} »` : ''}. ${cheaper && cheaper !== winner ? `Si le budget prime, ${cheaper.name} (${cheaper.pricingLocalized.fr}) est l'option la plus abordable. ` : ''}Choisissez ${winner.name} pour le meilleur outil de ${tlCategory(cat, 'fr')} dans l'ensemble${wUse ? `, surtout pour « ${tl(wUse, 'fr')} »` : ''} ; préférez ${other.name} si ${oStrength ? `« ${tl(oStrength, 'fr')} »` : 'son prix ou ses fonctionnalités spécifiques'} compte davantage pour vous.`,
    es: `Nuestro veredicto experto: ${tie
      ...
      : `${winner.name} es la opción más sólida en general, con ${winner.rating}/5 frente a ${other.rating}/5 de ${other.name}`}${wStrength ? `, y destaca por «${tl(wStrength, 'es')}»` : ''}. ${cheaper && cheaper !== winner ? `Si el presupuesto es tu prioridad, ${cheaper.name} (${cheaper.pricingLocalized.es}) es la opción más económica. ` : ''}Elige ${winner.name} si quieres la mejor herramienta de ${tlCategory(cat, 'es')} en general${wUse ? `, especialmente para ${tl(wUse, 'es').toLowerCase()}` : ''}; elige ${other.name} si ${oStrength ? `«${tl(oStrength, 'es')}»` : 'su precio o funciones específicas'} importa más para ti.`,
    ar: `رأي خبرائنا: ${tie
      ? `${t1.name} و${t2.name} متقاربان جدًا (${t1.rating}/5 لكليهما)، لذا يعتمد الاختيار على أولوياتك`
      : `${winner.name} هو الخيار الأقوى عمومًا بتقييم ${winner.rating}/5 مقابل ${other.rating}/5 لـ ${other.name}`}${wStrength ? `، ويتميّز بـ «${tl(wStrength, 'ar')}»` : ''}. ${cheaper && cheaper !== winner ? `إذا كانت الميزانية أولويتك، فإن ${cheaper.name} (${cheaper.pricingLocalized.ar}) هو الخيار الأوفر. ` : ''}اختر ${winner.name} إذا أردت أفضل أداة ${tlCategory(cat, 'ar')} إجمالاً${wUse ? `، خاصةً لـ ${tl(wUse, 'ar')}` : ''}؛ واختر ${other.name} إذا كان ${oStrength ? `«${tl(oStrength, 'ar')}»` : 'سعره أو ميزاته'} أهم بالنسبة لسير عملك.`,
  };
}
